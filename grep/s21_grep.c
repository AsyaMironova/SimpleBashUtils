...
void parser(int argc, char *argv[], Flags *A) {
  int opt;
  while ((opt = getopt_long(argc, argv, "e:ivclnhsf:o", NULL, 0)) !=
         -1) {  // e:ivclnhsf:o - все флаги
    switch (opt) {
      case 'e':
        A->e = 1;
...
void output(char *argv[], Flags *A) {
  ...
  if (file != NULL) {
    while (fgets(A->str, BUFFER, file) != NULL) {
      ...
      if (!match) counter++;                             // flag  c
      if (A->v) match = !match;                          // flag v
      if (!match && !A->l && !A->c && !A->n && !A->o) {  // grep
        printf("%s", A->str);
